...
def train_detection_model(data_yaml_path, epochs=100, imgsz=640, batch_size=-1, space={}, scheduler={}):
    '''
    训练检测模型
    ...
    for i, result in enumerate(result_grid):
        ...
        with open(f"./results/trial{now_time}.txt", "w") as f:
            f.write(f"Trial #{i}: Configuration: {result.config}, Last Reported Metrics: {result.metrics}")
...
if __name__ == "__main__":
    ...
    train_detection_model(path, epochs=8, imgsz=640, space=search_space, scheduler=scheduler)